...
class Listeners(commands.Cog):
    ...
    async def on_ready(self):
        ...
        # Check if Logobot is still a member of a guild or if the verification channel and/or verification roles have
        # been deleted
    ...
    async def on_guild_join(self, guild):
        sql = ("CREATE TABLE IF NOT EXISTS s%d (domain varchar(100), verifiedrole bigint, unverifiedrole bigint, "
               "unverifiedchannel bigint, userid BIGINT PRIMARY KEY, code INT, tagline varchar(100), verified BOOLEAN);"
               % guild.id)
        self.cursor.execute(sql)
        ...
        try:
            ...
            try:
                await self.initchannel(guild)
            except PermissionError:
                guild.owner.send(
                    "It seems that I don't have permission to create channels, which means you (or an admin) will "
                    "either need to give me that permission and call the 'initializechannel' command or manually set "
                    "a channel for verification and call the 'setchannel' command for it.")
        except PermissionError:
            guild.owner.send(
                "It seems that I don't have permission to manage roles, which means you (or an admin) will either "
                "need to give me that permission and call the 'initializeroles' command or manually set the verified "
                "and (optionally) unverified roles with the 'setrole' command. You may also need to do the same for "
...
class AdminCommands(commands.Cog):
    ...
    async def initializechannel(self, ctx, name='Verification'):
        ...
        if not ctx.channel.permissions_for(ctx.author).administrator:
            ...
            await ctx.send(
                "A role for verification must be designated with either the 'initializeroles' command or the "
                "'setrole' command. A role for unverified users is optional.")
            return
    ...
    async def ichannel_error(self, ctx, error):
        if isinstance(error, PermissionError):
            await ctx.send(
                'I do not have permission to do that. I need to be granted the "Manage Channels" permission. Or, '
                'you can manually designate a channel for verification with the "setchannel" command.')
    ...
    async def setchannel(self, ctx, id=None):
        ...
        if id is not None:
            try:
                ...
                if channel is None:
                    await ctx.send(
                        "The designated channel does not exist. If you are confused on how to get the id of a "
                        "channel, you must enable Discord Developer Mode in your personal settings; then, "
                        "when you right click on a channel, you'll have the option to copy its ID.")
                    return
            except TypeError:
    ...
    async def initializeroles(self, ctx, verifiedname="Verified", unverifiedname="Unverified"):
        ...
        sql = "update s%d set verifiedrole = '%s', set unverifiedrole = '%s' where userid = 0;" % (
            ctx.guild.id, verifiedrole, unverifiedrole)
        self.cursor.execute(sql)
        self.connection.commit()
    ...
    async def iroles_error(self, ctx, error):
        if isinstance(error, PermissionError):
            await ctx.send(
                'I do not have permission to do that. I need to be granted the "Manage Roles" permission. Or, '
                'you can manually designate a verified and (optionally) unverified role by copying their IDs and '
                'using the "setrole" command.')
    ...
    async def setrole(self, ctx, verifiedroleid, unverifiedroleid=None):
        ...
        try:
            ...
            if role is None:
                await ctx.send(
                    "The designated verified role does not exist. If you are confused on how to get the id of a role, "
                    "you must enable Discord Developer Mode in your personal settings; then, when you right click on "
                    "a role, you'll have the option to copy its ID.")
                return
            else:
        ...
        if unverifiedroleid is not None:
            try:
                ...
                if unverifiedrole is None:
                    await ctx.send(
                        "The designated unverified role does not exist. If you are confused on how to get the id of a "
                        "role, you must enable Discord Developer Mode in your personal settings; then, when you right "
                        "click on a role, you'll have the option to copy its ID.")
                else:
